...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const feeCents = Math.max(0, Math.round(await getTenantNumberSetting(tenantId, "NO_SHOW_FEE_CENTS", 4500)));
    ...
    if (methods.length) {
      ...
      if (!charge.ok) return NextResponse.json({ error: charge.message || "The card-on-file charge was not successful. The No-Show Fee has not been posted to the customer balance." }, { status: 402 });
      ...
      await db.$transaction(async tx => {
        ...
        await tx.$executeRaw`UPDATE Ticket SET noShowFeeCents=${feeCents},noShowFeeProcessedAt=CURRENT_TIMESTAMP(3),noShowFeePaymentId=${paymentId},status='CANCELLED',closedAt=CURRENT_TIMESTAMP(3) WHERE id=${ticketId} AND tenantId=${tenantId}`;
      });
      await writeAudit({ tenantId, actorUserId: session.user.id, entityType: "TICKET", entityId: ticketId, customerId: ticket.customerId, action: "NO_SHOW_FEE_CHARGED", summary: `Charged $${(feeCents/100).toFixed(2)} No-Show Fee to card on file for ticket #${ticket.orderNumber}.`, details: { feeCents, paymentId, chargeId: charge.chargeId, last4: method.last4, noShowStatus: "NO_SHOW" } });
    ...
    await db.$executeRaw`UPDATE Ticket SET noShowFeeCents=${feeCents},noShowFeeProcessedAt=CURRENT_TIMESTAMP(3),status='CANCELLED',closedAt=CURRENT_TIMESTAMP(3) WHERE id=${ticketId} AND tenantId=${tenantId}`;
    await writeAudit({ tenantId, actorUserId: session.user.id, entityType: "TICKET", entityId: ticketId, customerId: ticket.customerId, action: "NO_SHOW_FEE_TO_ACCOUNT", summary: `Added $${(feeCents/100).toFixed(2)} No-Show Fee to customer account balance for ticket #${ticket.orderNumber}.`, details: { feeCents, ledgerId, noShowStatus: "NO_SHOW" } });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to process No-Show fee." }, { status: 400 });
  }
}
