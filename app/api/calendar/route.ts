...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  if(tenantId&&locationId){
   ...
   try{
    const[r]=await connection.query(`SELECT t.id,t.orderNumber,t.scheduledStart,t.status,t.totalCents,c.firstName customerFirstName,c.lastName customerLastName,tp.petId,p.name petName,p.breed petBreed,tl.id lineId,tl.description lineDescription,ta.userId assignmentUserId,ta.role assignmentRole,u.firstName assignmentFirstName,u.lastName assignmentLastName FROM Ticket t JOIN Customer c ON c.id=t.customerId LEFT JOIN TicketPet tp ON tp.ticketId=t.id LEFT JOIN Pet p ON p.id=tp.petId LEFT JOIN TicketLine tl ON tl.ticketId=t.id LEFT JOIN TicketAssignment ta ON ta.ticketId=t.id LEFT JOIN User u ON u.id=ta.userId WHERE t.tenantId=? AND t.locationId=? AND t.scheduledStart>=? AND t.scheduledStart<? AND t.status<>? ORDER BY t.scheduledStart ASC,tl.sortOrder ASC`,[tenantId,locationId,start,end,"CANCELLED"]);
    const map=new Map<string,any>();for(const row of r as any[]){let t=map.get(row.id);if(!t){t={id:row.id,orderNumber:row.orderNumber,scheduledStart:row.scheduledStart,status:row.status,totalCents:Number(row.totalCents)||0,customer:{firstName:row.customerFirstName,lastName:row.customerLastName},pets:[],lines:[],assignments:[]};map.set(row.id,t)}if(row.petId&&!t.pets.some((x:any)=>x.pet.id===row.petId))t.pets.push({pet:{id:row.petId,name:row.petName,breed:row.petBreed}});if(row.lineId&&!t.lines.some((x:any)=>x.id===row.lineId))t.lines.push({id:row.lineId,description:row.lineDescription});if(row.assignmentUserId&&!t.assignments.some((x:any)=>x.user.id===row.assignmentUserId&&x.role===row.assignmentRole))t.assignments.push({user:{id:row.assignmentUserId,firstName:row.assignmentFirstName,lastName:row.assignmentLastName},role:row.assignmentRole})}tickets=[...map.values()]
