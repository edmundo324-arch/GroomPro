...
async function tenantFrom(request: NextRequest) {
  ...
  const first=await db.tenant.findFirst({orderBy:{createdAt:"asc"},select:{id:true}});
  return first?.id||"";
}

export async function GET(request: NextRequest) {
  const tenantId = await tenantFrom(request);
  if (!tenantId) return NextResponse.json({ error: "Business setup is not complete yet." }, { status: 503 });
