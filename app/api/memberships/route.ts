...
export async function GET(request: NextRequest) {
  ...
  if(customerId){ const memberships=await db.$queryRaw<any[]>`SELECT * FROM Membership WHERE tenantId=${tenantId} AND customerId=${customerId} ORDER BY createdAt DESC`; return NextResponse.json({memberships}); }
  if(petId){ const memberships=await db.$queryRaw<any[]>`SELECT * FROM Membership WHERE tenantId=${tenantId} AND petId=${petId} ORDER BY createdAt DESC`; return NextResponse.json({memberships}); }
  return NextResponse.json({error:"customerId or petId is required."},{status:400});
}
...
export async function PATCH(request: NextRequest) {
  const tenantId=tenantFrom(request); const session=await sessionFor(request,tenantId); if(!session)return NextResponse.json({error:"Employee PIN is required."},{status:401});
  try{
    const b=await request.json(); const id=String(b.membershipId||""); const action=String(b.action||"").toUpperCase(); if(!id||!["CANCEL","PAUSE","RESUME"].includes(action))return NextResponse.json({error:"A valid membership and action are required."},{status:400});
    const rows=await db.$queryRaw<any[]>`SELECT * FROM Membership WHERE id=${id} AND tenantId=${tenantId} LIMIT 1`; if(!rows.length)return NextResponse.json({error:"Membership not found."},{status:404}); const now=new Date();
    if(action==="RESUME") await db.$executeRaw`UPDATE Membership SET status='ACTIVE',pausedAt=NULL,pauseRequestedAt=NULL,finalBillingAt=NULL,updatedAt=NOW(3) WHERE id=${id} AND tenantId=${tenantId}`;
    else{
      const notice=await db.$queryRaw<any[]>`SELECT value FROM TenantSetting WHERE tenantId=${tenantId} AND settingKey='VIP_CANCELLATION_NOTICE_DAYS' LIMIT 1`; const days=Math.max(0,Number(notice[0]?.value||30)); const finalBilling=addDays(now,days); const status=action==="CANCEL"?"CANCELLATION_REQUESTED":"PAUSE_REQUESTED";
      if(action==="CANCEL") await db.$executeRaw`UPDATE Membership SET status=${status},cancelRequestedAt=${now},finalBillingAt=${finalBilling},updatedAt=NOW(3) WHERE id=${id} AND tenantId=${tenantId}`;
      else await db.$executeRaw`UPDATE Membership SET status=${status},pauseRequestedAt=${now},finalBillingAt=${finalBilling},updatedAt=NOW(3) WHERE id=${id} AND tenantId=${tenantId}`;
