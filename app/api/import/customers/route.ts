...
export async function POST(request:NextRequest){
 ...
 for(let i=0;i<rows.length;i++){const row=rows[i],firstName=field(row,"firstName"),lastName=field(row,"lastName"),email=field(row,"email").toLowerCase(),phone=field(row,"phone"),normalized=normalizePhone(phone);if(!firstName&&!lastName){errors.push({row:i+2,message:"Customer first or last name is required."});continue}if(!normalized&&!email){errors.push({row:i+2,message:"A phone number or email is required to safely match/create a customer."});continue}
  const existing=normalized?await db.customerPhone.findFirst({where:{tenantId,normalized},include:{customer:true}}):email?await db.customer.findFirst({where:{tenantId,email}}):null;const petNames=[1,2,3,4,5].map(n=>field(row,`dog${n}Name`)).filter(Boolean);preview.push({row:i+2,action:existing?"MATCH":"CREATE",customer:`${firstName} ${lastName}`.trim(),pets:petNames});if(body.dryRun)continue;
  let customer=existing?.customer||existing;if(customer)customersMatched++;else{customer=await db.customer.create({data:{tenantId,firstName:firstName||"Imported",lastName:lastName||"Customer",email:email||null}});customersCreated++}
