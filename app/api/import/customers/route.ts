...
export async function POST(request:NextRequest){
 ...
 for(let i=0;i<rows.length;i++){const row=rows[i],firstName=field(row,"firstName"),lastName=field(row,"lastName"),email=field(row,"email").toLowerCase(),phone=field(row,"phone"),normalized=normalizePhone(phone);if(!firstName&&!lastName){errors.push({row:i+2,message:"Customer first or last name is required."});continue}if(!normalized&&!email){errors.push({row:i+2,message:"A phone number or email is required to safely match/create a customer."});continue}
  const existingCustomer=normalized?(await db.customerPhone.findFirst({where:{tenantId,normalized},include:{customer:true}}))?.customer||null:email?await db.customer.findFirst({where:{tenantId,email}}):null;const petNames=[1,2,3,4,5].map(n=>field(row,`dog${n}Name`)).filter(Boolean);preview.push({row:i+2,action:existingCustomer?"MATCH":"CREATE",customer:`${firstName} ${lastName}`.trim(),pets:petNames});if(body.dryRun)continue;
  let customer=existingCustomer;if(customer)customersMatched++;else{customer=await db.customer.create({data:{tenantId,firstName:firstName||"Imported",lastName:lastName||"Customer",email:email||null}});customersCreated++}
  if(normalized){const phoneExists=await db.customerPhone.findFirst({where:{tenantId,normalized}});if(!phoneExists)await db.customerPhone.create({data:{tenantId,customerId:customer.id,number:phone,normalized,isPrimary:true}})}
  const address=field(row,"address"),address2=field(row,"address2"),city=field(row,"city"),state=field(row,"state"),postalCode=field(row,"postalCode");if(address||address2||city||state||postalCode)await db.customer.update({where:{id:customer.id},data:{address1:address||undefined,address2:address2||undefined,city:city||undefined,state:state||undefined,postalCode:postalCode||undefined}});
  for(let n=1;n<=5;n++){const dogName=field(row,`dog${n}Name`);if(!dogName)continue;const dogBreed=field(row,`dog${n}Breed`),samePet=await db.pet.findFirst({where:{tenantId,customerId:customer.id,name:dogName,active:true}});if(!samePet){await db.pet.create({data:{tenantId,customerId:customer.id,name:dogName,breed:dogBreed||null}});petsCreated++}}
 }
 if(!body.dryRun)await writeAudit({tenantId,actorUserId:session.user.id,entityType:"CUSTOMER",entityId:tenantId,action:"IMPORT",summary:`Imported customer data: ${customersCreated} created, ${customersMatched} matched, ${petsCreated} pets added.`,details:{rows:rows.length,errors:errors.length,locationId:body.locationId||null}});return NextResponse.json({ok:true,dryRun:!!body.dryRun,preview,errors,totals:{rows:rows.length,customersCreated,customersMatched,petsCreated}})
}
