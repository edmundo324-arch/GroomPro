...
export async function chargeCloverToken(params:{source:string;amountCents:number;description:string;externalReference:string;receiptEmail?:string|null}) : Promise<CloverChargeResult> {
  ...
  const response=await fetch(`${base}/v1/charges`,{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json",Authorization:`Bearer ${token}`,"idempotency-key":randomUUID()},body:JSON.stringify({amount:params.amountCents,currency:"usd",source:params.source,description:params.description,external_reference_id:params.externalReference,receipt_email:params.receiptEmail||undefined})});
